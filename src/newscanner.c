    ...
    while (1)
    {
        ...
        case FSM_HEXNUMBER_2:
            if (isxdigit(c))
            {
                ...
                { /*success*/ /*TODO is this correct format? ma to vyrobit token ci nie?*/
                    {
                        if (!dynamic_string_add_char(&stringbuffer, '0'))
                        {
                            return memoryerror;
                        }
                    }
                    {
                        if (!dynamic_string_add_char(&stringbuffer, 'x'))
                        {
                            return memoryerror;
                        }
                    }
                    {
                        if (!dynamic_string_add_char(&stringbuffer, buffedchar))
                        {
                            return memoryerror;
                        }
                    }
